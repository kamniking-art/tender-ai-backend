    ...
    try:
        ...
        if _cfs_early is not None:
            ...
            try:
                ...
                if _extracted_fs is not None:
                    # Inject tender.region so FitScorer._region() can match service_regions
                    if tender.region:
                        _extracted_fs._tender_region = tender.region
                    # Inject financial profile signals for FitScorer._economics() / _risk()
                    if decision.expected_margin_pct is not None:
                        _extracted_fs._estimated_margin_pct = float(decision.expected_margin_pct)
                    if risk_score is not None:
                        _extracted_fs._risk_score = risk_score
